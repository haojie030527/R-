...
获取校准曲线截距与斜率：
验证集：
y_obs <- as.numeric(test_std$是否 == "Yes")
logit_p <- log(test_eval$p_val / (1 - test_eval$p_val))

# 拟合校准模型（逻辑回归，以实际结局 ~ logit(预测概率)）
cal_model <- glm(y_obs ~ logit_p, family = binomial)

# 提取系数
cal_intercept <- coef(cal_model)[1]   # 截距
cal_slope     <- coef(cal_model)[2]   # 斜率
# 计算95%置信区间
ci_intercept <- confint(cal_model)[1, ]
ci_slope     <- confint(cal_model)[2, ]

# 打印结果
cat("校准截距 (Calibration intercept):", round(cal_intercept, 4),
    "95% CI [", round(ci_intercept[1], 4), ",", round(ci_intercept[2], 4), "]\n")
cat("校准斜率 (Calibration slope):", round(cal_slope, 4),
    "95% CI [", round(ci_slope[1], 4), ",", round(ci_slope[2], 4), "]\n")

训练集：
y_obs <- as.numeric(train_std$是否 == "Yes")
logit_p <- log(train_eval$p_val / (1 - train_eval$p_val))
